...
const LeadGeneration = z.object({ id: z.string(), project: z.object({ id: z.string(), name: z.string() }).strict(), fixedAt: z.string().datetime({ offset: true }) }).strict();
...
const AcademyWebinarInList = z.object({ id: z.string(), title: z.string(), description: z.string(), img: Image }).strict();
const AcademyWebinar = z.object({ id: z.string(), title: z.string(), description: z.string(), img: Image, content: z.array(InfoBlock) }).strict();
...
const endpoints = makeApi([
	...
	{
		method: "get",
		path: "/api/kpi/lead-generation",
		alias: "getLeadGeneration",
		requestFormat: "json",
		parameters: [
			{
				name: "year",
	...
	},
	{
		...
		response: LeadGenerationQuarterPassed,
		errors: [
			{
				status: 401,
				description: `Пользователь не авторизован`,
