...
export function SalesPage() {
  ...
  const { data: uploadedYears, isLoading: isUploadedYearsLoading } = useQuery(
    getRetailingUploadedYearsQueryOptions({ type: "sales", partnerId }),
  );
  ...
  const { data: retailingList, isLoading: isRetailingListLoading } = useQuery(
    getRetailingListQueryOptions({ type: "sales", partnerId, year }),
  );
  ...
  return (
    <Section className="col-span-full">
      <SectionHeader>
        <h1 className="text-h1 text-heading-h2">Продажи</h1>
      ...
      <SectionContent>
        ...
        <Section space="lg" className="rounded-main bg-card ~px-6/14 ~py-6/9">
          <SectionHeader>
            <h2 className="text-h2 text-heading-h3">Продажи по месяцам</h2>
          </SectionHeader>
          <SectionContent>
            <RetailingMonthsTable
              data={retailingList}
